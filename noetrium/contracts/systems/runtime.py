""" AUTO-GENERATED by scripts/generate_downstream_contracts.py.
System: runtime
Authority: runtime_state
Canonical authority: runtime
Node kind: authority
Downstream surface: public
API exports: available
This module is regenerated from the canonical registry and API exports.
"""

from noetrium_platform.infrastructure.lifecycle.api import (
    LifecycleComponent,
    LifecycleEvidence,
    LifecyclePhase,
    LifecycleSpec,
    SystemIdentity,
    SystemPort,
    SystemSpec,
)

from noetrium_platform.infrastructure.lifecycle.api.component import (
    LifecycleComponent as component__LifecycleComponent,
    LifecycleEvidence as component__LifecycleEvidence,
    LifecyclePhase as component__LifecyclePhase,
    LifecycleSpec as component__LifecycleSpec,
)

from noetrium_platform.infrastructure.lifecycle.api.contracts import (
    SystemIdentity as contracts__SystemIdentity,
    SystemPort as contracts__SystemPort,
    SystemSpec as contracts__SystemSpec,
)

from noetrium_platform.infrastructure.lifecycle.api.ports import (
    SystemPort as ports__SystemPort,
    SystemSpec as ports__SystemSpec,
)

SYSTEM_KEY = 'runtime'
PACKAGE_PREFIX = 'noetrium_platform.infrastructure.lifecycle'
__all__ = ('LifecycleComponent', 'LifecycleEvidence', 'LifecyclePhase', 'LifecycleSpec', 'SystemIdentity', 'SystemPort', 'SystemSpec', 'component__LifecycleComponent', 'component__LifecycleEvidence', 'component__LifecyclePhase', 'component__LifecycleSpec', 'contracts__SystemIdentity', 'contracts__SystemPort', 'contracts__SystemSpec', 'ports__SystemPort', 'ports__SystemSpec')
