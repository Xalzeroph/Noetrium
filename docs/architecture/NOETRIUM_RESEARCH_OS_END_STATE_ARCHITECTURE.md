...
4. MachineExecutor 拥有提案到提交的唯一转换路径：解释器只产生 TransitionProposal，Journal 才接受事实。
...
2. MachineExecutor 可绑定一个家族描述，在解释器执行前拒绝未准入的 command kind，并验证 MachineIdentity 与家族 kind 一致。
...
3. ResearchRunSession 把 Research Run contract 接到 MachineExecutor，只引用 journal head，不复制机器状态；它提供运行记录、隔离分叉、比较和研究包组装。
4. DirectoryResearchPackageStore 以不可变规范包保存研究记录；读取时验证 package digest、身份和整体编码，损坏或身份漂移 fail closed。
5. programmable_machine_families() 提供 Experiment、Run、Agent Turn、Memory、Environment、Evaluation 六个可运行参考解释器。它们只解释领域状态，不能直接写 Journal；提交仍由 MachineExecutor 唯一接管。
...
ProgramLock + NIR → MachineExecutor → Journal/Snapshot/Outbox → ResearchRunSession → Record/Compare/Fork/Package
...
nsh/SDK source
  ...
  -> local MachineExecutor validation
...
4. MachineExecutor 在 proposal 前与 journal append 前各做一次 assert_held；lease
...
- 最终提交只发生在本地 MachineExecutor 的 authority fencing 临界区内。
...
- `nsh` 仅负责 source-to-manifest，运行和 commit 仍归 MachineExecutor；
...
1. 所有 Machine state mutation 必须经过 MachineExecutor + journal；
...
- worker 资格测试覆盖 replay、错误签名、越权 scope、错误 program_digest 和过期 proposal；
  worker 仍只产生 candidate，MachineExecutor 才能提交 journal fact；
...
- 旧 single-agent harness、重复 public facade、旧 component registration 及其大规模
  生成 schema 重写未恢复：它们与 MachineExecutor 唯一 authority、system catalog 唯一
...
- Journal 对新增字段执行 exact-field、canonical encoding、digest、前驱链和子机链接完整性校验；`MachineExecutor.replay` 提供 journal-backed replay。
...
- nsh 新增 experiment compile、run start/inspect/pause/resume/complete/fail/replay、evidence 和 artifact 查询投影；运行与提交继续由 MachineExecutor 负责。
...
- `ResourceBudget`、`ResourceCapacity` 和 `ResourceSchedulerPort` 提供显式资源 admission；`MachineExecutor` 可在解释执行前申请并在成功或异常路径释放 reservation，禁止通过隐式全局配额运行。
...
本节只追加当前实现状态，不修改此前历史节。architecture gate 将 `ArtifactBlobStorePort` 与 `RunArtifactStorePort` 作为 kernel 的内容寻址持久化端口；它们分别约束内容读取校验和运行产物记录，具体实现仍通过显式 store 注入，不能由 facade 或全局 registry 偷换。
...
- 能力索引、资源监督和层级监督均不增加第二个 state/journal/facade authority：事实
  提交仍归 MachineExecutor，Machine fact 仍归 Journal，外部 effect 仍归 EffectIntentJournal。
...
## 67. 2026-09-16 authority-consolidation supersession: resource ownership

This section supersedes the earlier kernel-local `ResourceSchedulerPort` / `DirectoryResourceScheduler` design.
The Machine kernel owns deterministic state transition and Journal acceptance only. It does **not** own CPU,
RAM, GPU, token, concurrency-slot, placement, admission, or resource-lease truth.

The canonical resource path is now:

`ResearchExecutionPool / execution admission -> ComputeSchedulerPort -> ResourceLeasePort -> provider`

`ComputeSchedulerPort` owns compute candidate selection, allocation, renewal, expiry reconciliation and release.
`ResourceLeasePort` owns generic resource holder identity, TTL, fencing generation and lease lifecycle.
Experiment `ResourcePolicy` remains scientific intent and must not contain live physical host/GPU facts.
Machine execution may only occur after the outer execution/resource authorities admit it; `MachineExecutor`
never acquires or releases resource capacity internally.

This boundary is intentional: replacing local placement with Ray, Slurm, Kubernetes/Kueue, Volcano or another
provider must not change Machine semantics, MethodProgram identity, or Journal replay behavior.
