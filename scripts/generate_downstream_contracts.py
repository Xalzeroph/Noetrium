...
SCHEMA = "noetrium-downstream-contracts.v2"
...
def build_surfaces(root: Path) -> tuple[SystemSurface, ...]:
    ...
    for system_key, descriptor in catalog.items():
        ...
        rows.append(SystemSurface(
            ...
            authority=(
                descriptor["authority"] if descriptor["node_kind"] == "authority" else None
            ),
            canonical_authority=descriptor["canonical_authority"],
...
def render_facade(surface: SystemSurface) -> str:
    ...
    lines = [
        ...
        f"Canonical authority: {surface.canonical_authority}",
        f"Node kind: {surface.node_kind}",
...
def render_catalog(root: Path, surfaces: tuple[SystemSurface, ...]) -> bytes:
    ...
    for surface in surfaces:
        row: dict[str, Any] = {
            ...
            "canonical_authority": surface.canonical_authority,
            "node_kind": surface.node_kind,
            "owns": surface.owns,
...
def render_markdown(root: Path, surfaces: tuple[SystemSurface, ...]) -> bytes:
    ...
    for surface in surfaces:
        ...
        lines.extend([
            ...
            f"- Authority: {surface.authority or 'none'}",
            f"- Canonical authority: {surface.canonical_authority or 'none'}",
            f"- Node kind: {surface.node_kind}",
            f"- Owns: {surface.owns}",
