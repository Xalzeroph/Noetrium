...
SCHEMA = "noetrium.research-program.projection.v3"
AUTHORITY = "generated_from_program_control_benchmark_catalog_and_reproduction_catalog"
_TOKEN = re.compile(r"[a-z][a-z0-9_.-]*")
REPRODUCTION_LIFECYCLES = frozenset({"catalogued", "protocol_bound", "runnable", "pilot", "matched_reproduction", "blocked", "paper_only", "artifact_only"})
...
def _reproduction_packages(
    value: object,
    path: str,
    findings: list[ResearchProgramFinding],
) -> tuple[Mapping[str, Any], ...]:
    rows = _objects(value, path, findings)
    package_ids: list[str] = []
    for index, row in enumerate(rows):
        base = f"{path}[{index}]"
        if set(row) != {"package", "lifecycle"}:
            findings.append(
                ResearchProgramFinding(
                    "INVALID_REPRODUCTION_PACKAGE_SHAPE",
                    base,
                    "expected exactly package and lifecycle",
                )
            )
        package = _token(row.get("package"), f"{base}.package", findings)
        lifecycle = _text(row.get("lifecycle"), f"{base}.lifecycle", findings)
        if lifecycle and lifecycle not in REPRODUCTION_LIFECYCLES:
            findings.append(
                ResearchProgramFinding(
                    "INVALID_REPRODUCTION_LIFECYCLE",
                    f"{base}.lifecycle",
                    lifecycle,
                )
            )
    ...
    if len(package_ids) != len(set(package_ids)):
        findings.append(
            ResearchProgramFinding(
                "DUPLICATE_REPRODUCTION_PACKAGE",
                path,
                "package identities must be unique per method",
            )
        )
    return rows
...
def validate_research_program(document: Mapping[str, Any], registry: Mapping[str, Any]) -> tuple[ResearchProgramFinding, ...]:
    ...
    if document.get("authority") != AUTHORITY:
        findings.append(ResearchProgramFinding("AUTHORITY_MISMATCH", "authority", f"expected {AUTHORITY}"))
    ...
    for index, row in enumerate(methods):
        ...
        if "reproduction_status" in row:
            findings.append(
                ResearchProgramFinding(
                    "LEGACY_REPRODUCTION_STATUS_FORBIDDEN",
                    f"{base}.reproduction_status",
                    "reproduction lifecycle is package-owned; aggregate status is forbidden",
                )
            )
        reproductions = _reproduction_packages(
            row.get("reproduction_packages", []),
            f"{base}.reproduction_packages",
            findings,
        )
        if not reproductions:
            findings.append(ResearchProgramFinding(
                "ACTIVE_METHOD_WITHOUT_REPRODUCTION",
                f"{base}.reproduction_packages",
                "active research program methods must be projected from typed reproduction packages",
            ))
        ...
        if any(row.get("lifecycle") == "matched_reproduction" for row in reproductions) and not evidence:
            findings.append(
                ResearchProgramFinding(
                    "MATCHED_WITHOUT_EVIDENCE",
                    f"{base}.evidence_refs",
                    "matched reproductions require evidence refs",
                )
            )
    ...
    for index, row in enumerate(benchmarks):
        ...
        _string_tuple(row.get("environment_kinds"), f"{base}.environment_kinds", findings)
