...
def _uncovered_public_api_findings(
    ...
    *,
    ...
    for base in (root / "noetrium_platform",):
        if not base.exists():
            continue
...
def audit(root: Path) -> tuple[SystemSurfaceAudit, ...]:
    ...
    # Topology ownership is broader than downstream exposure.  A metadata-only
    # provider/projection/tool can own typed internal APIs without generating a
    # downstream facade. Audit every registered package API for ownership; then
    # validate generated public surfaces independently.
    owned_api_modules = tuple(
        module
        for descriptor in registry.values()
        for module in _api_modules(root, str(descriptor["package_prefix"]))
    )
    covered_sources = frozenset(module.source for module in owned_api_modules)
    covered_object_ids: set[int] = set()
    for module in owned_api_modules:
    ...
    for key, descriptor in registry.items():
        ...
        if downstream_surface == "metadata_only":
            # metadata_only controls generated downstream ABI only. It says
            # nothing about whether the internal topology node may have children
            # or provide capabilities to other platform systems.
            if symbol_count:
                findings.append(SurfaceFinding(
                    "METADATA_ONLY_SYSTEM_EXPORTS_GENERATED_API", key,
                    str(descriptor.get("package_prefix", "")),
                    f"metadata-only system leaked {symbol_count} generated symbols",
                ))
