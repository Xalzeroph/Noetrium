...
class ReflexionProtocolError(ValueError):
...
def accept_action_candidate(text: str) -> str | None:
    ...
    if "\n" in normalized or "\r" in normalized:
        raise ReflexionProtocolError("Reflexion action candidate must contain exactly one line")
    return normalized
...
def extract_failed_scenario(trial_log: str) -> str:
    ...
    if marker not in trial_log:
        raise ReflexionProtocolError("Reflexion failed trial is missing task marker")
    scenario = trial_log.rsplit(marker, 1)[-1].strip()
    if not scenario:
        raise ReflexionProtocolError("Reflexion failed trial scenario is empty")
    return scenario
...
def render_reflection_prompt(
    *,
    ...
    if not state.should_reflect:
        raise ReflexionProtocolError("Reflexion reflection is invalid for solved/skipped task")
