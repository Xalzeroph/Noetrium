...
def select_saycan_skill(
    ...
    skill_order = tuple(llm_log_scores)
    ...
    max_score = max(row[3] for row in raw_rows)
    ...
    rows = tuple(
        SayCanSkillScore(
            ...
            normalized_score=min(max(combined / max_score, 0.0), 1.0),
        )
        for skill, language, affordance, combined in raw_rows
    )
    # Python's max() keeps the first encountered item on ties, matching the
    # released notebook's max(combined_scores, key=combined_scores.get).
    selected = max(rows, key=lambda row: row.normalized_score)
