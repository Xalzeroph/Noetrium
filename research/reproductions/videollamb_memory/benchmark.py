...
def build_videollamb_egoschema_public_cut(
    ...
    *,
    ...
    if task_set.benchmark_id != EGOSCHEMA_BENCHMARK_ID:
        raise ValueError("VideoLLaMB EgoSchema benchmark authority drifted")
    if len(task_set.selected_tasks(EGOSCHEMA_PUBLIC_SPLIT)) != EGOSCHEMA_PUBLIC_COUNT:
        raise ValueError("VideoLLaMB EgoSchema public cut must contain 500 tasks")
    return task_set


__all__ = ["build_videollamb_egoschema_public_cut"]
