...
class UniversalMethodMachine:
    ...
    def run(
        ...
        *,
        ...
        while state.sequence < self._max_steps:
            ...
            try:
                next_state = self._advance(state, node, node_result, operation_effects)
                self._validate_state(program, runtime, next_state.state, node.node_id)
            except Exception as exc:
                return self._result(
                    MethodRunStatus.FAILED, program, runtime, state,
                    failure=f"{type(exc).__qualname__}: {exc}",
            ...
            state = self._commit_authoritative_transition(
                program, runtime, node.node_id, node_result, operation_effects, next_state
            )
    ...
    def _initial_state(self, program: MethodProgram, runtime: MethodRuntimeContext,
        ...
        if runtime.transitions is not None:
            authoritative = runtime.transitions.open(
                run_id=runtime.execution.run_id,
                program=program,
                initial_state=freeze_json(initial_state or {}),
                resume=resume,
            ...
            return self._execution_state_from_authoritative(authoritative)
        if resume:
            raise ValueError("method resume requires Machine transition authority; checkpoint receipts are not execution truth")
    ...
    def _request(self, program: MethodProgram, runtime: MethodRuntimeContext, input_value: object,
        ...
        if runtime.child_machines is not None and runtime.transitions is None:
            raise RuntimeError(
                "nested child Machines require Machine transition authority"
            )
        parent_machine_id = (
            None
            if runtime.child_machines is None
            else runtime.transitions.machine_id
        )
    ...
    def _agent_request(node: object, request: MethodNodeRequest) -> MethodAgentRequest:
        projector = getattr(node, "agent_view", None)
        ...
        selector = getattr(node, "agent_target", None)
        if callable(selector):
            agent_id = selector(request)
            if not isinstance(agent_id, str) or not agent_id.strip():
                raise ValueError("dynamic agent selector must return canonical non-empty text")
            allowed = getattr(node, "agent_targets", ())
    ...
    @staticmethod
    def _capability_id(node: object, request: MethodNodeRequest) -> str:
        selector = getattr(node, "capability_target", None)
        if callable(selector):
            capability_id = selector(request)
            if not isinstance(capability_id, str) or not capability_id.strip():
                raise ValueError(
                    "dynamic capability selector must return canonical non-empty text"
                )
            allowed = getattr(node, "capability_targets", ())
            if capability_id not in allowed:
                raise ValueError(
                    "dynamic capability selector chose undeclared capability: "
                    f"{capability_id!r}"
                )
    ...
    def _save_checkpoint(self, program: MethodProgram, runtime: MethodRuntimeContext,
        ...
        del current_node  # authoritative cursor is read from Machine state
        if runtime.transitions is None:
            if self._checkpoints is not None:
                raise RuntimeError("method checkpoint receipt store requires Machine transition authority")
            return None
        checkpoint = runtime.transitions.checkpoint(
            self._authoritative_state_from_execution(program, runtime, state)
        )
        if self._checkpoints is not None:
    ...
    @staticmethod
    def _execution_state_from_authoritative(authoritative: MethodAuthoritativeState) -> _ExecutionState:
        return _ExecutionState(
            authoritative.current_node,
            authoritative.sequence,
            authoritative.state,
            authoritative.previous_value,
            authoritative.events,
            dict(authoritative.visit_counts),
            authoritative.effect_receipts,
            authoritative.checkpoint_value,
        )

    @staticmethod
    def _authoritative_state_from_execution(
        program: MethodProgram,
        runtime: MethodRuntimeContext,
    ...
    def _commit_authoritative_transition(
        self,
        program: MethodProgram,
        runtime: MethodRuntimeContext,
        executed_node: str,
        result: MethodNodeResult,
        operation_effects: tuple[EffectReceipt, ...],
        state: _ExecutionState,
    ) -> _ExecutionState:
        if runtime.transitions is None:
            return state
        emitted_events = (*result.events, MethodEvent(f"node:{executed_node}", result.value))
        accepted = runtime.transitions.commit(MethodTransitionRecord(
            post_state=self._authoritative_state_from_execution(program, runtime, state),
            executed_node=executed_node,
            emitted_events=emitted_events,
            emitted_effect_receipts=operation_effects,
            emitted_child_links=result.child_links,
            interrupt=result.interrupt,
        ))
        return self._execution_state_from_authoritative(accepted)

    def _commit_authoritative_control(
        self,
        program: MethodProgram,
        runtime: MethodRuntimeContext,
        state: _ExecutionState,
        ...
        *,
        ...
        if runtime.transitions is None:
            return state
        accepted = runtime.transitions.commit_control(MethodControlRecord(
            state=self._authoritative_state_from_execution(program, runtime, state),
            status=status,
            failure=failure,
    ...
    def _result(self, status: MethodRunStatus, program: MethodProgram, runtime: MethodRuntimeContext,
        ...
        state = self._commit_authoritative_control(
            program, runtime, state, status,
            failure=failure, failure_code=failure_code, failure_phase=failure_phase,
        )
