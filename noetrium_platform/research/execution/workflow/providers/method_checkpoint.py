...
class JsonMethodCheckpointStore(MethodCheckpointStorePort):
    ...
    schema_version = "method-checkpoint-receipt.v3"
    ...
    def _encode(checkpoint: MethodCheckpoint) -> dict[str, object]:
        return {
            ...
            "checkpoint_value": thaw_json(checkpoint.checkpoint_value),
            "checkpoint_id": checkpoint.checkpoint_id,
        }

    @staticmethod
    def _decode(value: dict[str, Any]) -> MethodCheckpoint:
        if value.get("schema_version") != "method-checkpoint-receipt.v3":
            raise ValueError("unsupported method checkpoint receipt schema")
        checkpoint = MethodCheckpoint(
            ...
            machine_revision=value["machine_revision"],
            machine_commit_id=value.get("machine_commit_id"),
            state_digest=value["state_digest"],
            checkpoint_value=value.get("checkpoint_value"),
        )
        if value.get("checkpoint_id") != checkpoint.checkpoint_id:
            raise ValueError("method checkpoint receipt digest mismatch")
        return checkpoint
