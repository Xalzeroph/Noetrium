...
class ParticipantMessageOperations:
    """Workflow-owned operation boundary for participant message effects.

    Participant topology/schedule defines causal intent, the router is a replaceable
    provider, OperationDispatchPort owns effect execution, and Agent Turn facts are
    only candidate facts for the enclosing Machine Journal. No layer here owns a
    second durable message ledger.
    """
    ...
    ) -> tuple[ParticipantMessageRouteReceipt, OperationResult[ParticipantMessageOperationOutput], tuple[AgentTurnFact, AgentTurnFact]]:
