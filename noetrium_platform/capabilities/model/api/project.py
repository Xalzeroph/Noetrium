...
class ProjectModelBinding:
    ...
    request_tokenization_digest: str | None
    capability_id: str = "generation"
    input_schema_id: str = "model.generation.request.v1"
    output_schema_id: str = "model.generation.response.v1"
    ...
    def __post_init__(self) -> None:
        ...
        if generation:
            ...
            _sha256(
                self.request_tokenization_digest,
                "project model binding request_tokenization_digest",
            )
        else:
            ...
            if self.request_tokenization_digest is not None:
                raise ValueError("non-generation model binding must not carry request tokenization")
...
@dataclass(frozen=True, slots=True)
class ProjectModelBindingSet:
    """Frozen set of qualified bindings explicitly admitted for one requirement.

    The set does not route requests. It defines the complete scientific choice
    set from which a caller or provider policy may select without silently
    changing model/provider identity.
    """

    bindings: tuple[ProjectModelBinding, ...]
    requirement_digest: str = field(init=False)
    role: str = field(init=False)
    binding_set_digest: str = field(init=False)

    def __post_init__(self) -> None:
        if not isinstance(self.bindings, tuple) or not self.bindings:
            raise TypeError("project model binding set requires a non-empty tuple")
        if any(not isinstance(row, ProjectModelBinding) for row in self.bindings):
            raise TypeError("project model binding set must contain ProjectModelBinding")
        ordered = tuple(sorted(self.bindings, key=lambda row: row.digest()))
        digests = tuple(row.digest() for row in ordered)
        if len(digests) != len(set(digests)):
            raise ValueError("project model binding set must not contain duplicates")
        requirement_digests = {row.requirement_digest for row in ordered}
        roles = {row.role for row in ordered}
        protocols = {
            (row.capability_id, row.input_schema_id, row.output_schema_id)
            for row in ordered
        }
        if len(requirement_digests) != 1 or len(roles) != 1 or len(protocols) != 1:
            raise ValueError(
                "project model binding set members must satisfy one requirement/role/protocol"
            )
        object.__setattr__(self, "bindings", ordered)
        object.__setattr__(self, "requirement_digest", ordered[0].requirement_digest)
        object.__setattr__(self, "role", ordered[0].role)
        object.__setattr__(
            self,
            "binding_set_digest",
            canonical_digest(
                {
                    "requirement_digest": ordered[0].requirement_digest,
                    "role": ordered[0].role,
                    "binding_digests": digests,
                }
            ),
        )
    ...
    def binding_digests(self) -> tuple[str, ...]:
        return tuple(row.digest() for row in self.bindings)

    def contains(self, binding: ProjectModelBinding) -> bool:
        if not isinstance(binding, ProjectModelBinding):
            raise TypeError("project model binding set membership requires ProjectModelBinding")
        return binding.digest() in set(self.binding_digests)
    ...
    def selection_receipt(
        ...
        *,
        ...
        if not self.contains(binding):
            raise ValueError("selected model binding is outside the frozen admitted set")
        return ModelBindingSelectionReceipt(
            request_digest=request_digest,
            binding_set_digest=self.binding_set_digest,
            selected_binding_digest=binding.digest(),
...
@dataclass(frozen=True, slots=True)
class ModelBindingSelectionReceipt:
    """Evidence for the concrete binding used by one model request attempt."""
    ...
    def __post_init__(self) -> None:
        _sha256(self.request_digest, "model selection request_digest")
        _sha256(self.binding_set_digest, "model selection binding_set_digest")
        _sha256(self.selected_binding_digest, "model selection selected_binding_digest")
        if type(self.attempt_index) is not int or self.attempt_index <= 0:
            raise ValueError("model selection attempt_index must be positive")
        _text(self.reason_code, "model selection reason_code")
        if self.previous_selection_receipt_digest is not None:
            _sha256(
                self.previous_selection_receipt_digest,
                "model selection previous_selection_receipt_digest",
            )
        if not isinstance(self.evidence_refs, tuple) or any(
            not isinstance(ref, str) or not ref.strip() for ref in self.evidence_refs
        ):
            raise TypeError("model selection evidence_refs must contain non-empty strings")
        if len(set(self.evidence_refs)) != len(self.evidence_refs):
            raise ValueError("model selection evidence_refs must be unique")
        if self.attempt_index == 1 and self.previous_selection_receipt_digest is not None:
            raise ValueError("first model selection attempt cannot reference a predecessor")
        if self.attempt_index > 1:
            if self.previous_selection_receipt_digest is None:
                raise ValueError("later model selection attempt requires predecessor receipt")
            if not self.evidence_refs:
                raise ValueError("later model selection attempt requires causal evidence")
        object.__setattr__(
            self,
            "receipt_digest",
            canonical_digest(
...
class ProjectModelResponse:
    ...
    def __post_init__(self) -> None:
        ...
        if self.selection_receipt is not None:
            if not isinstance(self.selection_receipt, ModelBindingSelectionReceipt):
                raise TypeError("project model response selection_receipt must be typed")
            if self.selection_receipt.request_digest != self.request_digest:
