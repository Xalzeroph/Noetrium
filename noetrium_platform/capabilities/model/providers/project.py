...
class _QualifiedProjectModelClient:
    __slots__ = ("_binding", "_requirement", "_endpoint", "_model_requests", "_tokenization")
    ...
    def complete(self, request: ProjectModelRequest) -> ProjectModelResponse:
        ...
        if self._requirement.tool_schema_sha256 is not None:
            if (
                ...
                or envelope.tool_schema_bundle.content_sha256 != self._requirement.tool_schema_sha256
            ):
...
class QualifiedModelProjectProvider(ProjectModelProviderPort):
    ...
    def bind(self, requirement: ModelCapabilityRequirement) -> ProjectModelClientPort:
        ...
        try:
            ...
            raise ModelProjectBindingError(
                (_diagnostic(
                    ...
                    requirement,
                    ModelBindingDiagnosticCode.QUALIFIED_BINDING_UNAVAILABLE,
                    f"model request tokenization unavailable: {_exception_detail(exc)}",
                ),)
            ) from exc
        ...
        token_identity = tokenization.identity
        if (
            token_identity.model != binding.model
            or token_identity.model_stack_digest != binding.model_stack_digest
            or token_identity.tokenizer_sha256 != binding.tokenizer_sha256
            or token_identity.chat_template_sha256 != binding.chat_template_sha256
        ):
            raise ModelProjectBindingError(
                (_diagnostic(
                    self._profile,
        ...
        project_binding = ProjectModelBinding(
            ...
            request_tokenization_digest=token_identity.digest(),
