        ...
        "methods": [
            {
                ...
                "reproduction_packages": [
                    {"package": "method-bench", "lifecycle": "catalogued"}
                ],
                "priority": 1,
                "benchmark_ids": ["bench"],
...
def test_research_program_rejects_matched_reproduction_without_evidence() -> None:
    document = _document()
    document["methods"][0]["reproduction_packages"][0]["lifecycle"] = "matched_reproduction"
    findings = validate_research_program(document, {"model/request": {}})
    assert any(row.code == "MATCHED_WITHOUT_EVIDENCE" for row in findings)



def test_research_program_rejects_legacy_aggregate_reproduction_status() -> None:
    document = _document()
    document["methods"][0]["reproduction_status"] = "scaffolded"
    findings = validate_research_program(document, {"model/request": {}})
    assert any(row.code == "LEGACY_REPRODUCTION_STATUS_FORBIDDEN" for row in findings)
...
def test_research_program_rejects_active_method_without_reproduction() -> None:
    document = _document()
    document["methods"][0]["reproduction_packages"] = []
    findings = validate_research_program(document, {"model/request": {}})
    assert any(row.code == "ACTIVE_METHOD_WITHOUT_REPRODUCTION" for row in findings)
