...
def _runtime(journal: InMemoryMachineJournal) -> MachineRuntime:
    lock = ProgramLock(
        _digest("1"),
        _digest("2"),
        _digest("3"),
    ...
    return MachineRuntime(
        identity=MachineIdentity("agent-turn:1", MachineKind.AGENT, "2", "generation:1"),
        program=MachineProgramRef(_digest("7"), "agent.turn.state.v2", "agent", "2", lock),
        journal=journal,
    )
