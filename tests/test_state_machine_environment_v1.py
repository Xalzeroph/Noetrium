...
def test_state_machine_session_reopens_directly_from_environment_machine_journal() -> None:
    assembly = _assembly()
    context = ExecutionContext("run", "trace", "span", task_id="task")
    first = assembly.runtime.open_session(
        assembly.implementation,
        session_id="shared-session",
        services=object(),
    )
    first.act(ActionRequest("action-1", "increment", {"amount": 2}, context))
    first_cut = first.machine_cut
    assert first_cut is not None

    reopened = assembly.runtime.open_session(
        assembly.implementation,
        session_id="shared-session",
        services=object(),
    )
    assert reopened.observe(context).payload["state"]["value"] == 2
    assert reopened.diagnostics()["known_action_ids"] == 1
    assert reopened.machine_cut is not None
    assert reopened.machine_cut.revision > first_cut.revision


def test_state_machine_live_truth_is_not_held_in_legacy_session_fields() -> None:
    session = _session()
    assert not hasattr(session, "_state")
    assert not hasattr(session, "_actions")
    assert not hasattr(session, "_observation_sequence")
    assert session.machine_cut is not None
