...
def _envelope(
    ...
    *, body: object | None = None, prompt_digest: str = D["prompt"],
    ...
    return ModelRequestEnvelope(
        ...
        request_body=ArtifactBlobRef(canonical_digest(visible_body), 2, "application/json"),
        tool_schema_bundle=ArtifactBlobRef(tool_digest, 2, "application/json"),
    )
...
def test_model_request_context_budget_is_enforced_by_exact_bound_tokenization_before_endpoint_call() -> None:
    ...
    request = ProjectModelRequest(
        requirement.digest(),
        _envelope(client.binding.model, body=body),
        body,
    )
